...
const validIndex = content.indexOf('.vp-wishlist-card__cart-btn:hover {');
if (validIndex !== -1) {
    // Keep up to the valid section, add the rest manually
    const validPart = content.slice(0, validIndex) +
        `.vp-wishlist-card__cart-btn:hover {
...
`;
    ...
    fs.writeFileSync(cssPath, validPart);
    console.log('Fixed CSS file successfully!');
} else {
    console.error('Could not find anchor point in CSS');
}
